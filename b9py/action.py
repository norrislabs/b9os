...
ACTION_STATUS_FEEDBACK = 4
ACTION_STATUS_RESULT = 5
...
class ActionServer(object):
    ...
    def feedback(self, status_code: int, status_data):
...
class ActionClient(object):
    def __init__(self, b9core: b9py.B9, action_topic, namespace, feedback_cb):
        self._b9 = b9core
        self._topic = action_topic
        self._namespace = namespace
        self._nodename = self._b9.nodename
        ...
        if feedback_cb:
            self._status_sub = self._b9.create_subscriber("action/" + action_topic + "/status",
                                                          self._feedback_closure(feedback_cb),
                                                          self._namespace)

    # Start - sends a ANY or NULL message to server and returns a B9Status with an ANY message result
    ...
    def _feedback_closure(self, callback):
        def feedback_callback(topic, msg: b9py.Message):
